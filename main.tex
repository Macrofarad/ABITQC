...
%\begin{figure}[H] %This keeps the code in the same place % note footnote doesn't play well with figure
	...
	\newline In the case of our example circuit above, after the initial states are set, the order of operations is as follows~\footnote{For this example, the $\rightarrow$ symbol is being used to represent the advancement of time between columns}:\newline 
	\fbox{$ Q_{1} = no-op; Q_{2}= no-op$}
	\fbox{$\rightarrow $}
	\fbox{$ Q_{1} = Gate "!"; Q_{2}= no-op$}
	\fbox{$\rightarrow $}
	\fbox{$ Q_{1} = nop; Q_{2}= no-op$}
	\fbox{$\rightarrow $}
