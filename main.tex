...
A more advanced concept that is still core to quantum computing is the idea of reversible logic gates. Normally when it comes to digital logic inputs and outputs are decoupled. Consider the following digital logic circuit:\newline % for spacing
\newline % for spacing
...
\draw (xory.output) -- node[above]{$f_1$} ($(xory) + (3, 0)$);
% f1 is $\overline{x + \bar x + \bar y + z}$
\end{tikzpicture}
