...
\usepackage[hidelinks]{hyperref} % allows for hyperlinks as well as makes underscores in a URL not break the document
	% Hidelinks Option removes the ugly boxes from clickable links
...
\begin{document}
	...
	\textcolor{mygray}{
			...
			\caption{Photograph of Bristlecone courtesy of Google~\cite{coverImage}}\label{titleImage} %https://ai.googleblog.com/search/label/Quantum%20Computing
...
\tableofcontents % yep, that's the whole code for a ToC
\newpage % See: https://www.latex-tutorial.com/tutorials/table-of-contents/
...
\subsection{Decomposition of a Two Qubit Swap} % Note: Matrix for easy usage can be found: https://commons.wikimedia.org/wiki/File:Qcircuit_SWAP.svg
...
\begin{multicols}{2} 
	...
	\[\Qcircuit @C=1.4em @R=1.8em @!R { %rows and columns, pretty standard
...
\subsection{Decomposition of a Quantum Fourier Transform} % Make sure to provide sources
...
\bibitem{coverImage} % This is not a proper source, but it'll do as a placeholder until google quantum AI team can get back to me  
	...
	% Email the address at %https://www.dwavesys.com/resources/media-resources to get permission and the data required to properly cite this as it's pretty impossible to find barring the mit journal in japanese which has it and doesn't link or use full citation
	%Possibly Larry Goldstein?
...
BitHacks - For binary numbers, ways to speed up computation or reduce op count
